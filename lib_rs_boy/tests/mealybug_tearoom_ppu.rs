...
mod test {
    ...
    fn ppu(rom: &str) {
        let rom = load_file(Path::new(TEST_DIR_PATH).join(rom.to_owned()+".gb").as_path()).unwrap();
        let mut gb = GameBoy::new();
        gb.load_rom(rom);

        let img: DynamicImage = ImageReader::open(Path::new(TEST_DIR_PATH).join(rom+"_dmg_blob.png").as_path()).unwrap()
            .decode().unwrap();
